---
title: "Illustration of MEFISTO on simulated Poisson data with a temporal covariate"
author:
- name: "Britta Velten"
...
This vignette illustrates the use of MEFISTO with a Poisson likelihood. Note that this is only applicable to tailored data sets and in general we recommend the use of a suitable pre-processing followed by a Gaussian likelihood. See also [out tutorial on data pre-processing](https://raw.githack.com/bioFAM/MEFISTO_tutorials/master/MEFISTO_preprocessing.html) for details.

# Temporal data: Simulate an example data set
To illustrate the MEFISTO method in MOFA2 we simulate a small example data set with 4 different views and one covariates defining a timeline using `make_example_data` with Poisson likelihoods. The simulation is based on 4 factors, two of which vary smoothly along the covariate (with different lengthscales) and two are independent of the covariate.
...
dd <- make_example_data(sample_cov = time, n_samples = N,
                        ...
                        lscales = c(0.5, 0.2, 0, 0), likelihood = "poisson")
...
Before training, we can specify various options for the model, the training and the data preprocessing. If no options are specified, the model will use the default options. See also `get_default_data_options`, `get_default_model_options` and `get_default_training_options` to have a look at the defaults and change them where required. For illustration, we only use a small number of iterations. We additionally set the likelihood of the model to poisson as this was used to simulate the data.
...
model_opts$likelihoods <- rep("poisson", sm@dimensions$M)
...
outfile = file.path(tempdir(),"model_poisson.hdf5")
